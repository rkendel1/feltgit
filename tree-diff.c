...
/*
 * Diff state blobs from two state commits.
 * Creates a single diff entry for the state blob.
 */
void diff_state_oid(const struct object_id *old_oid,
		   const struct object_id *new_oid,
		   const char *base_str, struct diff_options *opt)
{
	struct diff_filespec *old_spec, *new_spec;
	struct diff_filepair *pair;
	struct diff_queue_struct *q = &diff_queued_diff;

	/* Create filespec objects for the state blobs */
	old_spec = alloc_filespec("state");
	new_spec = alloc_filespec("state");
	...
	pair = diff_queue(q, old_spec, new_spec);
	pair->score = 0;
}

/*
 * Diff state blob for a root state commit.
 * When showing a root state commit, diff against empty.
 */
void diff_root_state_oid(const struct object_id *new_oid,
			const char *base, struct diff_options *opt)
{
	diff_state_oid(NULL, new_oid, base, opt);
}
